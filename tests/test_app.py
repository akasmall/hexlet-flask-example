...
def test_template_has_post():
    with requests.Session() as s:
        ...
        assert ('method="post"' in response.text) or (
            "method='post'" in response.text)
...
def test_delete_post():
    with requests.Session() as s:
        ...
        id_ = response.headers['X-ID']
        response = s.post(
            urljoin(BASE_URL, f'/posts/{id_}/delete'), allow_redirects=False)
        response = s.get(urljoin(BASE_URL, '/posts'))
...
def test_get_method_not_allowed():
    with requests.Session() as s:
        ...
        # GET method not allowed for delete forms
